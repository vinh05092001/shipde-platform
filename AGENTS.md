...
- **Human — product and merge owner:** resolves product decisions, triggers each controller gate, accepts residual risk and merges only after CI is green and Codex returns `PASS`. Under decision `HUMAN-DECISION-ORCHESTRATOR-CORE-PRIORITY-2026-09-08` and Work Item `TASK-AI-13`, the deterministic controller is authorized to execute an exact-HEAD auto-merge after `READY_FOR_HUMAN_MERGE` only when strict preflight passes (exact 40-character commit OID, required GitHub Actions `SUCCESS` checks, trusted `chatgpt-codex-connector[bot]` `PASS` verdict, zero unresolved review threads, and `expectedHeadOid` squash mutation). Human merge remains authoritative for the bootstrap of TASK-AI-13 itself and whenever any preflight signal fails.
