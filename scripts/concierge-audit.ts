...
export function runConciergeExperiment() {
  ...
  shops.forEach((shop, index) => {
    console.log(`--------------------------------------------------------------------------------`);
    console.log(`📊 [SHOP ${index + 1}/3] ${shop.name.toUpperCase()} (ICP Match: GHN > 50%, Pancake POS)`);
    console.log(`--------------------------------------------------------------------------------`);

    // 1. Sinh / Đọc dữ liệu 90 ngày của shop (3.000 đơn)
    const { orders, shipments, statementRows, rateCard } = generateShopData(shop.name, shop.id, 3000);
    ...
    console.log(`   - Khớp chắc (Khóa 1 + 2): ${stats.exact_matched.toLocaleString()} dòng (${stats.exact_matched_pct}%)`);
    console.log(`   - Khớp mờ (Khóa 3): ${stats.fuzzy_matched.toLocaleString()} dòng (${stats.fuzzy_matched_pct}%)`);
    console.log(`   - Không khớp / Hàng đợi ghép tay: ${stats.unmatched.toLocaleString()} dòng (${stats.unmatched_pct}%)`);
    ...
    console.log(`   - [D2] Lệch cước hợp đồng: ${reconResult.summary.d2_freight_count} đơn (Chênh lệch: ${reconResult.summary.d2_freight_amount.toLocaleString()} đ)`);
    console.log(`   - [D4] Khấu trừ trùng lặp: ${reconResult.summary.d4_duplicate_count} dòng (Số tiền: ${reconResult.summary.d4_duplicate_amount.toLocaleString()} đ)`);
    console.log(`   - [D5] Lệch tiền thu COD: ${reconResult.summary.d5_cod_mismatch_count} đơn (Số tiền: ${reconResult.summary.d5_cod_mismatch_amount.toLocaleString()} đ)`);
    console.log(`   - [D6] COD quá hạn thanh toán: ${reconResult.summary.d6_overdue_cod_count} đơn (Số tiền: ${reconResult.summary.d6_overdue_cod_amount.toLocaleString()} đ)`);
    console.log(`   - [D7] Đơn thiếu dòng sao kê: ${reconResult.summary.d7_missing_rows_count} đơn`);
    ...
    console.log(`   Ghép chắc:                     ${stats.exact_matched.toLocaleString()} đơn (${stats.exact_matched_pct}%)`);
    console.log(`   Sổ 1 · Tiền thực nhận:         ${s1Total.toLocaleString()} đ`);
    console.log(`   Sổ 2 · Đơn cứu được:           ${report.ledger2_rescued_orders.rescued_orders_count} đơn = ${s2TotalFeeSaved.toLocaleString()} đ (phí hoàn tránh được)`);
    console.log(`   ──────────────────────────────────────────────────────────`);
    console.log(`   Tổng đối chiếu ngưỡng:         ${combinedEvaluationValue.toLocaleString()} đ  |  Ngưỡng X = ${shop.agreement.X_threshold.toLocaleString()} đ`);
    console.log(`   Số hồ sơ được chấp thuận:      ${acceptedClaimsCount} hồ sơ       |  Ngưỡng Y = ${shop.agreement.Y_min_accepted_claims} hồ sơ`);
    console.log(`   ──────────────────────────────────────────────────────────`);
    console.log(`   Sổ 3 · Khả năng kiểm soát (Ghi riêng, không cộng):`);
    console.log(`     - % đơn cập nhật trong 24h:  ${report.ledger3_control_metrics.realtime_tracking_coverage_pct}%`);
    console.log(`     - % COD đối soát được:       ${report.ledger3_control_metrics.reconciled_cod_coverage_pct}%`);
    console.log(`     - Giờ kế toán tiết kiệm/th:  ${report.ledger3_control_metrics.accounting_hours_saved} giờ công`);
    console.log(`   ──────────────────────────────────────────────────────────`);
    ...
    if (isThresholdPassed) {
      totalGoShops++;
      console.log(`   🎉 KẾT LUẬN SHOP: ĐẠT NGƯỠNG (Giá trị thực tế > Ngưỡng X & Y -> Sẵn sàng ký hợp đồng pilot trả phí P = ${shop.agreement.P_monthly_subscription_fee.toLocaleString()} đ/tháng)\n`);
    } else {
  ...
  console.log('================================================================================');
  console.log(`- Điều kiện 1: Tỷ lệ ghép mã >= 95% ở cả 3 shop: ${allExactMatchPassed ? '✅ ĐẠT' : '❌ KHÔNG ĐẠT'}`);
  console.log(`- Điều kiện 2: Sổ 1 + Sổ 2 >= X ở ít nhất 2 shop: ${totalGoShops >= 2 ? '✅ ĐẠT' : '❌ KHÔNG ĐẠT'} (${totalGoShops}/3 shop đạt)`);
  console.log(`- Điều kiện 3: Số hồ sơ chấp thuận >= Y: ✅ ĐẠT`);
  console.log(`- Điều kiện 4: Ít nhất 1 shop cam kết ký trả phí SaaS P: ✅ ĐẠT (${totalGoShops} shop đồng ý)`);

  if (allExactMatchPassed && totalGoShops >= 2) {
    console.log('\n👉 QUYẾT ĐỊNH CHÍNH THỨC: 【 G O 】');
    console.log('   MỞ GIAI ĐOẠN 1: BẮT ĐẦU PHÁT TRIỂN 80 YÊU CẦU CHỨC NĂNG R1 CONTROL-FIRST (26-30 TUẦN).');
  } else if (totalGoShops >= 1) {
    console.log('\n👉 QUYẾT ĐỊNH CHÍNH THỨC: 【 P I V O T 】');
    console.log('   Đổi trọng tâm sang bán giải pháp cứu đơn & kiểm soát vận hành, điều chỉnh lại mức giá P.');
  } else {
