...
Single-writer collision protection across concurrent AO sessions and direct terminal sessions is guarded by `tools/ai-guard`. The Git pre-commit hook (`.githooks/pre-commit`) blocks commits when another active session holds the target branch. The hook is installed automatically via `pnpm install` (`prepare` script) or manually via `node tools/ai-guard/cli.js install`.
