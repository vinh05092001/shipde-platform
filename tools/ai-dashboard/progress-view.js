...
(function () {
  ...
  function statusBar(byStatus, total) {
    var segs = STATUS.filter(function (s) {
      return byStatus[s[0]];
    })
      .map(function (s) {
      ...
      .join('');
    var legend = STATUS.filter(function (s) {
      return byStatus[s[0]];
    })
      .map(function (s) {
        return (
    ...
    return (
      ...
      segs +
      '</div>' +
      '<div class="flex flex-wrap gap-x-4 gap-y-1 text-[11px] mt-2 opacity-80">' +
      legend +
      '</div>'
    );
  }
      ...
      .map(function (k) {
        ...
        return (
          ...
          (ha
            ? '<div style="height:' +
              ha +
              'px;width:10px;background:#8b5cf6;border-radius:2px"></div>'
            : '') +
          (hg
            ? '<div style="height:' +
              hg +
              'px;width:10px;background:#16a34a;border-radius:2px"></div>'
            : '') +
  ...
  function render(state) {
    ...
    body.appendChild(
      el(
        ...
        '<div class="flex items-baseline justify-between">' +
          '<h3 class="card-title text-sm">Tiến độ ' +
          w.total +
          ' đầu việc</h3>' +
    ...
    body.appendChild(
      el(
        'div',
        '',
        '<div class="text-[11px] font-bold uppercase tracking-wide opacity-60 mb-2">Theo slice</div>' +
          '<div class="grid gap-1.5 sm:grid-cols-2">' +
          sliceBars(bySlice, mergedPerSlice(w.items)) +
          '</div>'
      )
    );
  ...
  function load() {
    fetch('/api/state')
      ...
      .catch(function () {
        /* the panel keeps whatever it last drew */
      });
  }
