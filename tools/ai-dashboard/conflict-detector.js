...
function detectConflicts(registerData, gitData, aoData, githubData) {
  ...
  if ((registerStatus === 'READY_FOR_CODEX' || registerStatus === 'CHANGES_REQUIRED') && gitData?.dirtyCount > 0) {
    conflicts.push({
  ...
  if (githubData?.pullRequests && githubData.pullRequests.length > 0) {
    const matchingPr = githubData.pullRequests.find(pr =>
      pr.headRefName === activeBranch || (pr.title && pr.title.includes(activeWorkItemId))
    );
  ...
  if (aoData?.sessions && aoData.sessions.length > 0) {
    const activeAuthorSession = aoData.sessions.find(s => s.roleCategory === 'AUTHOR' && !s.isTerminated);
    if (activeAuthorSession && activeBranch && activeAuthorSession.branch) {
      if (activeAuthorSession.branch !== activeBranch && !activeAuthorSession.branch.includes(activeWorkItemId.toLowerCase())) {
        conflicts.push({
