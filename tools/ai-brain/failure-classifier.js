...
/**
 * Extracts inner HTTP status code from body text like "[402]: ..." or "[403]: ...".
 */
function extractInnerStatus(text) {
  if (!text) return null;
  const match = String(text).match(/[\[(]\s*(401|402|403|429)\s*[\])]/);
...
function classifyFailure(input) {
  ...
  const evidence = { exitCode, httpStatus, body: String(body || '').slice(0, 500), stderr: String(stderr || '').slice(0, 500) };

  // If outer status is 503, look for inner status in body (gateway pattern: "[402]: ...")
  const effectiveStatus = (httpStatus === 503) ? (extractInnerStatus(text) ?? httpStatus) : httpStatus;

  // Case 1: upstream credit exhausted (402 with "out of credit" / provider credit)
  if (effectiveStatus === 402 && /out of credit|credit.{0,20}exhaust|insufficient.*credit|provider.{0,20}credit/i.test(text)) {
  ...
  if (effectiveStatus === 402 && /MONTHLY_REQUEST_COUNT|monthly.{0,10}limit|monthly.{0,10}request/i.test(text)) {
    const resetMs = parseResetTime(text);
    return {
  ...
  if (effectiveStatus === 429 && /rate.?limit|too many requests|user_global_rate_limited/i.test(text)) {
    const resetMs = parseResetTime(text);
    return {
  ...
  // Case 5: upstream credential problem (401 unauthorized)
  if (effectiveStatus === 401 && /unauthorized|invalid.*token|authentication failed|invalid.*credential/i.test(text)) {
    return {
  ...
  if (httpStatus === 400 && /model.{0,20}not.{0,10}support|not.{0,10}support.{0,10}model/i.test(text)) {
    return {
  ...
  if ((httpStatus === 503 || exitCode !== 0) && /no capacity available|capacity.{0,10}unavailable|server.{0,10}capacity/i.test(text)) {
    const resetMs = parseResetTime(text);
    return {
