'use strict';
// AC-AI-22-07 — the three negative proofs must fail operationally (exit 2), not
...
// reason" are indistinguishable. Each subject is spawned with its working
// directory in a fresh temporary folder holding no repository, and every child
// must exit 2 with SOURCE_MISSING. Arguments are passed as an argv array.
const fs = require('fs');
const os = require('os');
...
const SUBJECTS = [
  'ac-22-02-status-divergence.js',
  'ac-22-04-proposed-state-removed.js',
  'ac-22-06-self-approval-detected.js',
...
const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'shipde-ac-22-07-outside-'));
let failures = 0;
try {
  for (const subject of SUBJECTS) {
