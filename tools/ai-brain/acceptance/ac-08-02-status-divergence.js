'use strict';
// AC-AI-08-02 — negative proof for the Control-table/register comparison.
//
// The check lives in `./lib/spec-status-alignment.js`, the same module the
// invariant `ac-08-01-status-alignment.js` requires: editing the rule changes
...
// 1, so the negative proof cannot pass by accident where nothing exists. That
// property is itself measured by `ac-08-14-outside-repository.js`.
const fs = require('fs');
const os = require('os');
const path = require('path');
const { STATUS_CELL, controlStatus, registerStatus } = require('./lib/spec-status-alignment');

const SPEC = 'docs/product-spec/work-items/TASK-AI-08.md';
const REG = 'docs/product-spec/docs/10-ai-collaboration/FEATURE-DELIVERY-REGISTER.csv';
const WORK_ITEM = 'TASK-AI-08';
...
const tmp = path.join(os.tmpdir(), 'shipde-ac08-02-' + process.pid + '.md');
