'use strict';
// AC-AI-32-03 — the invariant: TASK-AI-32's register row declares exactly one
// dependency (TASK-AI-31) and that dependency is really merged into `origin/main`.
//
// The rule lives in `./lib/dependency-merged.js`, the same module the negative
// proof `ac-32-04-dependency-unproven.js` requires. The register is read from
// disk and Git supplies the merge evidence. Run outside the repository it exits
// 2, so it cannot pass where nothing exists.
//
// This row exists because the register records TASK-AI-32 as
// `BLOCKED_DEPENDENCY` while TASK-AI-31 is already merged in Git reality: the
...
const EXPECTED_DEPENDENCY = 'TASK-AI-31';
