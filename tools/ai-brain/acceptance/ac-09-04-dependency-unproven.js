'use strict';
// AC-AI-09-04 — negative proof that the dependency rule refuses a dependency
// with no merge commit, having first proved the real row is accepted.
//
// The rule is `./lib/dependency-merged.js`, the same module
// `ac-09-03-dependency-merged.js` requires: editing the rule changes both the
// invariant and this proof. The real register is read from disk; a COPY is
// written to `os.tmpdir()` with only the `dependencies` cell of the TASK-AI-09
...
// Tamper a COPY: repoint only the TASK-AI-09 row's dependency cell.
...
const tmp = path.join(os.tmpdir(), 'shipde-ac09-04-' + process.pid + '.csv');
