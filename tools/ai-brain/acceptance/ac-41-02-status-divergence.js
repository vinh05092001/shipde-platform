'use strict';
// AC-AI-41-02 — negative proof for the Control-table/register comparison.
//
// The comparison lives in `./lib/spec-status-alignment.js`, the same module the
// invariant `ac-41-01-status-alignment.js` requires. This script reads the REAL
...
// property is measured by `ac-41-09-outside-repository.js`.
const fs = require('fs');
const os = require('os');
const path = require('path');
const { STATUS_CELL, controlStatus, registerStatus } = require('./lib/spec-status-alignment');

const SPEC = 'docs/product-spec/work-items/TASK-AI-41.md';
...
const tmp = path.join(os.tmpdir(), 'shipde-ac41-02-' + process.pid + '.md');
