'use strict';
// AC-AI-11-04 — negative proof that the dependency rule refuses a dependency
// with no merge commit, having first proved the real row is accepted.
//
// The rule is `./lib/dependency-merged.js`, the same module
// `ac-11-03-dependency-merged.js` requires: editing the rule changes both the
...
// TASK-AI-11 row repointed at an id that no commit names. Everything else in
...
// Tamper a COPY: repoint only the TASK-AI-11 row's dependency cell.
...
const tmp = path.join(os.tmpdir(), 'shipde-ac11-04-' + process.pid + '.csv');
