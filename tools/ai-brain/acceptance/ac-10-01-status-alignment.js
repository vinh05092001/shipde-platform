'use strict';
// AC-AI-10-01 - the invariant: TASK-AI-10's Control table and its delivery
// register row (row 143, the register's `delivery_order` column) carry the same
// status. The comparison lives in `./lib/spec-status-alignment.js`, the same
// module the negative proof `ac-10-02-status-divergence.js` requires, so the gate
// and the proof of the gate cannot drift apart.
//
// The row reads the real specification and the real register; there is no
// fixture. Run outside the repository it exits 2, never 1.
const fs = require('fs');
...
console.log(
  'Control status matches register row 143: ' +
