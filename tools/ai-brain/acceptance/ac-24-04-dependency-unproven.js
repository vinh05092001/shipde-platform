'use strict';
// AC-AI-24-04 — negative proof that the dependency rule refuses a dependency
// with no merge commit, having first proved the real row is accepted.
//
// The rule is `./lib/dependency-merged.js`, the same module
// `ac-24-03-dependency-merged.js` requires: editing the rule changes both the
// invariant and this proof. The real register is read from disk; a COPY is
// written to `os.tmpdir()` with only the `dependencies` cell of the TASK-AI-24
...
// Tamper a COPY: repoint only the TASK-AI-24 row's dependency cell.
...
const tmp = path.join(os.tmpdir(), 'shipde-ac24-04-' + process.pid + '.csv');
...
if (verdict.ok) {
  ...
  // Exit 2, never 0: a proof that stopped detecting must not report success.
  process.exit(2);
}
