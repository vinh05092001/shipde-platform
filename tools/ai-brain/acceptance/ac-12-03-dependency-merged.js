'use strict';
// AC-AI-12-03 — the invariant: TASK-AI-12's register row declares exactly one
...
// proof `ac-12-04-dependency-unproven.js` requires. The register is read from
// disk and Git supplies the merge evidence. Run outside the repository it exits
// 2, so it cannot pass where nothing exists.
//
// This row exists because the register records TASK-AI-12 as
// `BLOCKED_DEPENDENCY` while TASK-AI-11 is already merged in Git reality: the
...
const EXPECTED_DEPENDENCY = 'TASK-AI-11';
...
if (!verdict.ok) {
  console.error('DEPENDENCY_NOT_PROVEN: ' + EXPECTED_DEPENDENCY + ' ' + verdict.why);
  process.exit(1);
}
