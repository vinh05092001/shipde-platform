'use strict';
// AC-AI-30-02 — negative proof for the Control-table/register comparison.
//
// The check lives in `./lib/spec-status-alignment.js` and this script calls the
// SAME aggregate function the invariant `ac-30-01-status-alignment.js` calls
...
// so the negative proof cannot pass by accident where nothing exists. That property
// is itself measured by `ac-30-09-outside-repository.js`.
const fs = require('fs');
const os = require('os');
...
const tmp = path.join(os.tmpdir(), 'shipde-ac30-02-' + process.pid + '.md');
