'use strict';
// AC-AI-23-02 — negative proof for the Control-table/register comparison.
//
// The check lives in `./lib/spec-status-alignment.js` and this script calls the
// SAME aggregate function the invariant `ac-23-01-status-alignment.js` calls
...
// property is itself measured by `ac-23-17-outside-repository.js`.
const fs = require('fs');
const os = require('os');
...
const tmp = path.join(os.tmpdir(), 'shipde-ac23-02-' + process.pid + '.md');
