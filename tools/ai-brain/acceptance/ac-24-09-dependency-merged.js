'use strict';
// AC-AI-24-09 — the invariant: TASK-AI-24's register row declares exactly one
...
// proof `ac-24-10-dependency-unproven.js` requires. The register is read from
// disk and Git supplies the merge evidence. Run outside the repository it exits
// 2, so it cannot pass where nothing exists.
//
// The Work Item names TASK-AI-16's merge commit in prose; prose is an
// assertion, so this row proves the dependency from the repository instead of
// from the Work Item text or a register status.
const fs = require('fs');
