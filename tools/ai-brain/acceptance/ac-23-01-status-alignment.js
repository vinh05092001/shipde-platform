'use strict';
// AC-AI-23-01 — the invariant: TASK-AI-23's Control table and its
// delivery-register row (row 156, the register's `delivery_order` column) carry
// the same status. The comparison lives in `./lib/spec-status-alignment.js`, the
// same module the negative proof `ac-23-02-status-divergence.js` requires, so the
// gate and the proof of the gate cannot drift apart.
//
// The row reads the real specification and the real register; there is no
// fixture. Run outside the repository it exits 2, not 0, so it cannot pass where
// nothing exists.
const fs = require('fs');
const { controlStatus, statusAlignmentViolations } = require('./lib/spec-status-alignment');

const SPEC = 'docs/product-spec/work-items/TASK-AI-23.md';
...
console.log(
  'Control status matches register row 156: ' +
