'use strict';
// AC-AI-37-13 — the toolchain suite across ai-brain, ai-dashboard and ai-guard
// must report zero failures, and the run must be real.
//
// The stored row pinned `ℹ pass 458` and carried backslash-escaped quotes
// (`\"…\"`). Those quotes are not shell quoting: a shell hands node the literal
// quote characters, no glob expands, `node --test` matches nothing, and the row
// passed by printing `fail 0` over a zero-test run. A row asserting "fail 0"
// therefore proved nothing even before the count went stale (real count: 626).
//
...
// fail === 0. No pass count is pinned.
const fs = require('fs');
const os = require('os');
...
const emptyDir = fs.mkdtempSync(path.join(os.tmpdir(), 'shipde-ac-37-13-empty-'));
let vacuousOut = '';
try {
