...
| [pnpm](https://github.com/pnpm/pnpm)                                | `pnpm`                     | Install major 11 now; project pins exact version in `TASK-FOUND-02` | Requires Node 22+; do not use against the current npm prototype before migration          |
...
| [Playwright CLI](https://github.com/microsoft/playwright-cli)       | `playwright-cli`           | Install `@playwright/cli@0.1.2` globally                            | Token-efficient browser action execution for AI agents; distinct from Playwright Test     |
| [Gitleaks](https://github.com/gitleaks/gitleaks)                    | `gitleaks`                 | System binary / scanner                                             | Secret and credential audit gate; reject secrets before commit/merge                      |
| [Promptfoo](https://github.com/promptfoo/promptfoo)                 | `promptfoo`                | Install `promptfoo@0.111.0` globally                                | Prompt security assertions and LLM output evaluation suites                               |
| [Trivy](https://github.com/aquasecurity/trivy)                      | `trivy`                    | Windows package / winget                                            | Vulnerability and container image scanner                                                 |
...
The guarded installers are `scripts/ai/install-clis.ps1` and `scripts/ai/install-ecosystem.ps1`. Both preview by default, install only missing tools with `-Apply`, do not upgrade an installed CLI, do not sign in, and never read or store an API key.
...
| `TASK-FOUND-03` | [NestJS](https://github.com/nestjs/nest), [Prisma](https://github.com/prisma/prisma), [BullMQ](https://github.com/taskforcesh/bullmq), [OpenTelemetry](https://github.com/open-telemetry/opentelemetry-js) | API, worker, migration, queue foundations, and distributed tracing with local health checks   |
...
Additionally, on-demand Model Context Protocol (MCP) servers provide diagnostic interfaces:

| MCP Server          | Repository                           | Role                                     | Lifecycle                                 |
| ------------------- | ------------------------------------ | ---------------------------------------- | ----------------------------------------- |
| Chrome DevTools MCP | `ChromeDevTools/chrome-devtools-mcp` | Deep runtime CDP diagnostics             | Stopped by default; on-demand via profile |
| Playwright MCP      | `microsoft/playwright-mcp`           | Stateful interactive browser exploration | Stopped by default; on-demand via profile |
...
## Agent quality guidance and curated datasets

The following focused repositories inform implementation but do not replace Ship Dễ product specifications:
...
- [Vietnamese Provinces Database](https://github.com/thanglequoc/vietnamese-provinces-database): canonical administrative boundary dataset (`v2.1.0`) for address normalization; must be validated before ingestion.
