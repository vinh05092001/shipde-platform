...
Use one new Claude Code CLI session per Work Item after opening `C:\Users\gumac\AI\shipde-claude` (or the designated author worktree). Authenticate via native machine login (`claude auth login`). Replace all placeholders.
...
3. Confirm Assigned author is CLAUDE (or GEMINI/collaborative repair), dependencies are merged, the branch is correct and the worktree is clean.
...
```text
Read all current Codex/review findings on <PR_URL>. Fix every actionable finding for <WORK_ITEM_ID> on the existing branch only. Do not dismiss a finding without code/test evidence or a human-approved specification decision. Rerun the complete verification set, update PR evidence, push, set READY_FOR_CODEX and stop.
```
