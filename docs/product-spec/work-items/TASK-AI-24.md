...
| `AC-AI-24-05` | The real planner requires no launch capability, and `planDispatch` returns a plan that defers an item it cannot place | `node tools/ai-brain/acceptance/ac-24-05-planner-launches-nothing.js` | `0` | `PLANNER_LAUNCHES_NOTHING: scheduler.js requires no launch capability` | `tools/ai-brain/scheduler.js`, `tools/ai-brain/acceptance/lib/dispatch-executor-contract.js` |
...
| `AC-AI-24-07` | The controller's real launcher passes `spawn` with project, kind, name, branch, harness and prompt | `node tools/ai-brain/acceptance/ac-24-07-spawn-contract.js` | `0` | `SPAWN_CONTRACT_HOLDS: New-ShipDeAoSpawnArguments passes spawn with --project --kind --name --branch --harness --prompt` | `scripts/ai/control.ps1`, `tools/ai-brain/acceptance/lib/dispatch-executor-contract.js` |
...
- **`AC-AI-24-07` reads PowerShell source text.** It proves the launcher names every flag, not that AO accepts them; `control.ps1 -Action Test` covers the fixture spawn.
