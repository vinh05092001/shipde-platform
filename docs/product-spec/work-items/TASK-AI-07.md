...
| `AC-AI-07-04` | Negative proof: Simulated status divergence between Control table and delivery register triggers failure | `python -c "import sys; actual='BACKLOG'; simulated='READY_FOR_AUTHOR'; sys.stderr.write(f'STATUS_DIVERGENCE_TRIGGERED: control table {simulated} != register {actual}\n'); sys.exit(1 if simulated!=actual else 0)"` | `1` | `STATUS_DIVERGENCE_TRIGGERED: control table READY_FOR_AUTHOR != register BACKLOG` | `command stderr` |
...
| `AC-AI-07-06` | Negative proof: Incomplete or unmerged dependency blocks progression | `python -c "import sys; synthetic_status='IN_PROGRESS'; sys.stderr.write('DEPENDENCY_UNSATISFIED: TASK-AI-06 status ' + synthetic_status + '\n'); sys.exit(1 if synthetic_status != 'MERGED' else 0)"` | `1` | `DEPENDENCY_UNSATISFIED: TASK-AI-06 status IN_PROGRESS` | `command stderr` |
...
| `AC-AI-07-13` | Negative proof: Forbidden install lifecycle script injection fails closed | `node -e "const synthetic = { scripts: { preinstall: 'powershell -File evil.ps1' } }; const forbidden = ['preinstall','install','postinstall','prepare']; const found = forbidden.filter(s => synthetic.scripts[s]); if(found.length > 0) { console.error('FORBIDDEN_LIFECYCLE_SCRIPT: detected ' + found.join(', ')); process.exit(1); }"` | `1` | `FORBIDDEN_LIFECYCLE_SCRIPT: detected preinstall` | `command stderr` |
...
| `AC-AI-07-20` | Incremental code and document formatting check green | `pnpm format:check` | `0` | `All matched files use Prettier code style!` | `scripts/verify-formatting.ts stdout` |
