...
4. **`AC-AI-07-12` could not be run as written.** Its inline form needed a
   JavaScript `||`, which a markdown table cell stores escaped as `\|\|`.
   Measured: extracted verbatim from the table it dies on invalid syntax and
   exits `1`, not the `0` the row states; typed by hand with a real `||` it
   exits `0`. It is now a script, like the negative rows above.

5. **`AC-AI-07-11` passed four probes that do not exist.** It supplied
