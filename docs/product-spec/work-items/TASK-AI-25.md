...
- **F3 (blocking) — stale Pull Request description.** Rewritten in this round against the pushed diff: it names the delivered rule module, the nine acceptance rows, the 25 unit tests, the four mutation-proven `AC-AI-25-04` controls, the exact gate results and the `be96c17` CI run, and it states that no Codex verdict covers the current HEAD. The title was corrected in the same round from `Author the missing specification` — which named only the first commit on this branch — to the register's business outcome for delivery order 158, `Feed measured outcomes back into qualifiedRoles`, so the title and the description both describe the delivered rule module and harness. The Control table records `Pull Request` `#86`, so the Work Item and the open Pull Request it is delivered through agree.
