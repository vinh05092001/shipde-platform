...
| `AC-AI-25-04` | `node tools/ai-brain/acceptance/ac-25-04-rule-single-source.js`: single source | Exit `0` `SINGLE_SOURCE`: `feedback.js` and `AC-AI-25-03` both resolve `require('./acceptance/lib/role-feedback.js')` (or the production equivalent) to the identical file path; no second copy of the removal rule exists anywhere in the tree. `AC-AI-25-02` is not part of this check — it proves pre-existing gates are untouched and has no reason to load the new module. | stdout + exit |
...
**Rule-to-proof map.** `AI-25-R01`, `R03`, `R04`, `R06` are exercised by `AC-AI-25-03`/`06`. `R05` is exercised by `AC-AI-25-07`. `R07` has two named unit-test cases in `feedback.test.js`, since neither is a cross-file integration fact suited to a standalone `ac-25-*.js` row: `"a load with no source is refused SOURCE_MISSING"` and `"a stale operator-declared removal is refused as evidence for a new narrowing decision, reported EVIDENCE_STALE, without restoring the role"`. `AI-25-R02` (write surface), `R08` (separate ledgers, empty-not-error on a missing/corrupt store), `R09` (fixed history shape) and `R10` (fail-closed, no dispatch/cool/launch call) are likewise shape and isolation invariants over the single module `role-feedback.js`; they are proved by the deterministic unit tests in `node --test tools/ai-brain/test/feedback.test.js`, listed in Verification commands for exactly this reason.
