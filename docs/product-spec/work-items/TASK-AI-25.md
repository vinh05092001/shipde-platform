...
- **F3 (blocking) — stale Pull Request description.** Rewritten in this round against the pushed diff: it names the delivered rule module, the nine acceptance rows, the 25 unit tests, the four mutation-proven `AC-AI-25-04` controls, the exact gate results and the `be96c17` CI run, and it states that no Codex verdict covers the current HEAD. The Control table records `Pull Request` `#86`, so the Work Item and the open Pull Request it is delivered through agree.
