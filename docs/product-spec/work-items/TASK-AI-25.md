...
| `AC-AI-25-04` | `node tools/ai-brain/acceptance/ac-25-04-rule-single-source.js`: single source | Exit `0` `SINGLE_SOURCE`: two genuinely different requiring modules — this row (base `acceptance/`) and the production base `tools/ai-brain/feedback.js` — resolve `acceptance/lib/role-feedback.js` to the identical absolute path; no second copy of the removal rule exists anywhere in the tree. `AC-AI-25-02` is not part of this check — it proves pre-existing gates are untouched and has no reason to load the new module. | stdout + exit |
