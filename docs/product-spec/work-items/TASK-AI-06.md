...
| 16           | `HEAD`         | `READY_FOR_CODEX`                                                  | Repaired AO 0.12.12 session list adapter compatibility and silenced startup self-tests: (1) Added `ConvertFrom-ShipDeAoSessionResponse` supporting explicit observed AO 0.12.12 envelope (`{ data: [ ... ], meta: ... }`) and empty collections without pipeline array unrolling failures, maintaining fail-closed behavior on malformed, ambiguous, log-contaminated, or unrelated arrays; (2) Added behavioral fixtures covering observed AO 0.12.12 responses (non-empty, empty, bare array, wrapped result, top-level direct) and fail-closed rejections; (3) Silenced startup self-test output by locally intercepting `Write-Host` during `Assert-ShipDeSupervisorCompatibility` so no PASS, CHANGES_REQUIRED, PR data, or reconstructed-checkpoint messages leak before the real supervisor banner; (4) Added assertion verifying zero host output leaks during startup self-tests. |
