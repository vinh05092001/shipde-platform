...
| 13           | `39b9397dcacb` | `CHANGES_REQUIRED` by acceptance verification                      | Repaired all 13 findings: (1) trusted reviewer logins restricted to Codex app bot identity, removing repo owner; (2) AO session enumeration fails closed without swallowing errors; (3) dispatch intent persisted before AO side effects and acknowledged only upon delivery; (4) open PR GraphQL pagination decoupled from commit check contexts; (5) AGENTS.md and docs/product-spec/AGENTS.md restored to merge base and removed from allowed paths; (6) strict review JSON schema enforced without markdown fallback, using case-sensitive property checks and native integer types; (7) newly spawned AO sessions validated via Assert-ShipDeReusedAoSession before binding; (8) 9Router descendant process tree cleaned up recursively via CIM/taskkill; (9) Invoke-ShipDeResume halts immediately on durable BLOCKED; (10) Get-ShipDeMergedPullRequests paginated to exhaustion; (11) manifest AO version pin derived canonically with caller bypass prevention; (12) replaced source-text assertions with injected behavioral bootstrap regression tests; (13) added behavioral test for SPAWNING recovery with matching PR #9 and unrelated PR #8; reconciled review history and PR metadata. |
| 14           | `HEAD`         | `READY_FOR_CODEX`                                                  | Fixed PropertyNotFoundStrict in Invoke-ShipDeSupervisorLoop by normalizing the complete 27-property checkpoint schema with default null/native values across all creation and recovery paths; supported PR-only recovery when no reusable AO session exists (SessionId = $null); enabled non-interactive exact-HEAD Codex review execution without Read-Host prompts or duplicate worker spawning; added 4 acceptance behavioral regression tests covering missing SessionId under StrictMode, external lifecycle review, no interactive prompt, and no duplicate worker.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
