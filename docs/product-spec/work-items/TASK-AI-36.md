...
Three rows could not prove what they claimed, and seven rows assert a repository
...
**Replacement:** the claim is corrected, and the integration these seven rows
describe is implemented. Each of `AC-AI-36-01` through `-07` carries an explicit
`Met by this Work Item’s implementation (D-03 closed).` marker, and each one now
measures a committed artifact: the exact `"lefthook": "1.11.3"` root
devDependency pin (`-02`), the root `lefthook.yml` (`-04`), the manifest promotion
to `ADOPTED` / `BLOCKING_GATE` (`-01`), the workspace-pinned binary (`-03`, `-06`),
the generated `pre-commit` hook (`-05`) and its interception of a writer collision
(`-07`). The bespoke `core.hooksPath` override that these rows replace is retired,
so the Lefthook hook is the one Git runs.

### `D-04` — `AC-AI-36-07`: an invocation that never reached the repository under test

**Defect class:** unrunnable as written. The row asserts that the Lefthook
runner blocks a writer collision inside the temporary repository the row
builds. It invited the runner with

`cp.execSync('pnpm --dir "<root>" exec lefthook run pre-commit', { cwd: tempDir, env })`

and `pnpm --dir <root> exec` runs the child process in `<root>`, not in `cwd`
(`cwd` is accepted and then ignored). Measured:
`pnpm --dir "<root>" exec node -e "console.log(process.cwd())"` with `cwd` set
to the temporary repository prints `<root>`. With the process relocated to the
repository root, Lefthook read the **root** `lefthook.yml`: the run lists
`staged-secret-scan` and `writer-claim` — the root config’s two commands —
while the temporary config declares only `writer-claim`, skipped both for
having no staged files, and exited `0`. The row could therefore never reach the
collision it asserts, and its isolated claim store, synthetic branch and `HOME`
redirection were all inert.

**Replacement:** the runner is invoked through the root-installed pinned binary,
with the temporary repository as the working directory:

`cp.execSync('node "<root>/node_modules/lefthook/bin/index.js" run pre-commit', { cwd: tempDir, env })`

Measured with that invocation: exit `1`, and the combined output carries
`CHẶN GHI`, the synthetic branch and `existing-holder`, so the assertion is
exercised by the Lefthook runner rather than by the test harness. Every other
element of the stored command — the isolated claim store, the per-run branch and
the cleanup in `finally` — is unchanged.
...
| 6 | `607fc9e` | `READY_FOR_CODEX` | Post-merge follow-up completing the Work Item's hook-management scope after PR #96 merged: `tools/ai-guard` `install`/`uninstall`/`status` are now Lefthook-aware — `getHookStatus` answers from the hook Git actually executes (common hooks dir via `--git-common-dir`, or an explicit `core.hooksPath`) and requires `lefthook.yml` to still declare `node tools/ai-guard/cli.js check` before reporting `installed`; `installHook` retires any bespoke `core.hooksPath` override, refuses to run without the versioned `lefthook.yml` (Lefthook would invent an empty config), installs through the pinned workspace runner (`npx` fallback pinned to `lefthook@1.11.3`, bare `npx lefthook` never invoked per `AI-TOOL-11`) and re-verifies before claiming success; `uninstallHook` retires only the override and reports already-absent as success. Legacy `.githooks` wiring removed from the module, CLI and README (the `.ps1` scripts had already been promoted to Lefthook in PR #96); tests rewritten for the new state model (128/128 pass) and the full acceptance matrix re-run through Git's `sh.exe`: all 18 live AC rows (`AC-AI-36-01`–`15`, `17`–`19`; `16` removed in round 4) PASS, plus `pnpm lint` and `pnpm typecheck` green. |
...
- **Scope delivered.** The full scope of this Work Item: the `lefthook`
devDependency pin, `lefthook.yml`, the manifest promotion to `ADOPTED` /
`BLOCKING_GATE`, the staged secret scanner and its `cli.js` wiring, and the
`scripts/ai/*.ps1` and `AI-TOOLCHAIN-DECISIONS.md` updates. Two implementation
facts were not in the specification and are recorded here rather than assumed.
First, the `lefthook` npm package’s `postinstall` runs `lefthook install -f`,
which is exactly the implicit hook registration `AI-36-R02` forbids, so its build
script is denied in `pnpm-workspace.yaml` (`lefthook: false`) and the binary is
taken from the pinned `lefthook-windows-x64` optional dependency instead. Second,
Lefthook offers no way to install anywhere but the `core.hooksPath` directory, so
that override had to be retired for the hook to land in the repository’s git
common dir as `AC-AI-36-05` asserts.
