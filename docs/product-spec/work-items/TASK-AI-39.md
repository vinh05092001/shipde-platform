...
- Operational execution modes reconciling manifest truth: local evaluation under
  `local_mode: local-rule-evaluation (requires SNYK_TOKEN for rule catalog synchronization)`
  with graceful offline cached fallback vs. authenticated CI platform evaluation
  under the `SECURITY_REVIEW` and `PR_REVIEW` profiles via `snyk-api-outbound-https`
  to authorized endpoints (`https://api.snyk.io`, `https://app.snyk.io`).
...
- Define manifest promotion criteria for `tools/ecosystem-manifest.json`:
  - Formal 6-point checklist to transition `agent-scan` from `PENDING` to `ADOPTED` and
