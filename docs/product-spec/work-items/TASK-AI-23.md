...
| Pull Request | `#115` (carrier to `main`; it contains every commit of the closed `#85`) |
...
| `AC-AI-23-05` | Superseded and expired lessons stay out even when approved. The real seed's only non-null `expiry` (`LESSON-SINGLE-REPAIR-PER-HEAD`, `2026-12-31`) sits on an already-superseded record, so this row also builds one synthetic fixture record — schema-conforming (`scope: tooling`, checked with `lib/lesson-schema.js` before retrieval, exit `2` if not), approved, not superseded, `expiry` before the pinned retrieval date — to exercise the expiry-only path the live seed cannot reach; the row states this explicitly rather than implying the fixture is the live seed | `node tools/ai-brain/acceptance/ac-23-05-retrieval-staleness.js` | `0` | `RETRIEVAL_STALENESS_HOLDS: superseded and expired lessons stay out of retrieval` | `lesson-seed.json`, `ac-23-05-retrieval-staleness.js`, `lib/lesson-retrieval.js` |
...
| `AC-AI-23-05` | `node tools/ai-brain/acceptance/ac-23-05-retrieval-staleness.js` | `0` | `0` | `RETRIEVAL_STALENESS_HOLDS: superseded and expired lessons stay out of retrieval`; synthetic `LESSON-SYNTHETIC-EXPIRY-ONLY-FIXTURE` (zero schema violations) excluded expired, `LESSON-SINGLE-REPAIR-PER-HEAD` excluded superseded with that reason winning over expiry per `AI-23-R05` |
...
| `AC-AI-23-08` | `node tools/ai-brain/acceptance/ac-23-08-retrieval-nonconforming.js` | `0` | `0` | `RETRIEVAL_CONFORMANCE_HOLDS: a nonconforming approved lesson stays out of retrieval`; synthetic fixture uses `scope: tooling`, so its only schema defect is the missing `source_commit` (control exits `2` otherwise) |
...
| `AC-AI-23-15` | `pnpm format:check` | `0` | `0` | exit `0` on `#115`'s own CI run at `2534722`, whose diff against its base holds 3 files (2 format-governed scripts; `docs/` is in `.prettierignore`). The earlier 13-file count was the diff of the closed `#85` against `origin/main` |
...
The third-pass paragraph above is historical: it describes `4c22376`, whose `AC-AI-23-05` and `AC-AI-23-08` outputs head `2534722` has since rewritten, and its "12 commits ahead and 17 behind" is not the branch state today.

**Fourth pass at head `2534722` (`2026-09-19`, Linux container, clean checkout without `node_modules`).** The two changed scripts and every other row were re-executed at `2534722`. Fifteen rows reproduce the exit code and output recorded in the table: `01`/`03`/`05`/`07`/`08`/`09`/`13`/`14`/`16`/`17` exit `0` and the four negative proofs `02`/`04`/`06`/`10` exit `1`; `AC-AI-23-15` is taken from `#115`'s green CI `formatting` run, not a local run. `AC-AI-23-16` reports `files scanned: 324` on this host (the Windows figure is `323`). Two rows did not reproduce on this host and are not claimed here: `AC-AI-23-11` exits `1` with `SUITE_REGRESSION: fail 22 of 643 tests`, all 22 in `tools/ai-guard/test/verify-trivy.test.js`, which fails with `neither node_modules/tsx nor npm npx-cli.js is available`; `AC-AI-23-12` exits `1` with `Tổng: 7 lỗi` because the manifest's tools are not installed in this container. Neither suite nor manifest is touched by `2534722`, and `node --test tools/ai-brain/test/*.test.js` exits `0` with 393 pass and 0 fail. The independent review of `#115` measured both rows green on a host with dependencies installed. Measured against `origin/main` (`3f7336b`) the branch is 15 commits ahead and 18 behind. The commit that adds this paragraph changes only this file.
