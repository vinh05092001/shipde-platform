...
Written `2026-09-17`; evidence recorded `2026-09-19` at commit `543736d` on `spec/task-ai-23` and re-executed the same day at head `2e5cce4`, the three commits between being documentation-only; every exit code and output string below reproduced unchanged at that head. The harness is delivered in this PR: scripts `ac-23-01` through `ac-23-10`, `ac-23-17-outside-repository.js`, and shared modules `lib/lesson-retrieval.js` and `lib/dependency-delivered.js`, all under `tools/ai-brain/acceptance/`. Each row of the matrix above was therefore executed against this checkout, and the observed exit code and observed output are pasted below verbatim rather than asserted. All seventeen rows reproduced their stated exit code.
