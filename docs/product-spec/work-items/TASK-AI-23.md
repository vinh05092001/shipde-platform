...
| `AI-23-R03` | **Expired lessons stay out**: `expiry` before the retrieval date is excluded even when approved. The retrieval date is the wall-clock UTC date (`YYYY-MM-DD`) at the instant the gate runs, taken from the caller-supplied clock so a test can pin it; explicit `null` means not time-bounded and stays admissible. |
...
| `AI-23-R07` | **One shared module**: the admissibility predicate lives in exactly one place, `tools/ai-brain/acceptance/lib/lesson-retrieval.js`, and is `require`d by both the canonical gate under `tools/ai-brain/retrieval/` and every invariant row and negative proof in `tools/ai-brain/acceptance/ac-23-*.js`; no row and no gate file restates the predicate inline. |
...
| `AC-AI-23-04` | **Negative proof, must fail:** seed copy with the proposed record's `status` set to `approved` **and** `approved_by` set to a non-null reviewer id (isolating the status-leak check from the unrelated `approved_by` conformance requirement) leaks and is refused | `node tools/ai-brain/acceptance/ac-23-04-retrieval-leak.js` | `1` | `RETRIEVAL_LEAK: LESSON-BRAIN-UNTRUSTED-UNTIL-REVIEWED must not become retrievable by editing status alone` | `lesson-seed.json`, `ac-23-04-retrieval-leak.js`, `lib/lesson-retrieval.js`; stderr |
...
| `AC-AI-23-17` | The four negative proofs (`AC-AI-23-02`, `AC-AI-23-04`, `AC-AI-23-06`, `AC-AI-23-10`) exit 2, not 1, outside the repository, so the set grows with the negative-proof count instead of staying pinned at a stale literal | `node tools/ai-brain/acceptance/ac-23-17-outside-repository.js` | `0` | `OUTSIDE_REPOSITORY_PROBE: ac-23-02, ac-23-04, ac-23-06, ac-23-10 each exited 2 with SOURCE_MISSING outside the repository` | `ac-23-17-outside-repository.js`; stdout |
### Evidence notes
