...
| ------------ | ------------------------------------------ | ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1            | `bcc9094cfc8aecada9aef9febeb179da9ce87653` | CHANGES_REQUIRED | **Blocking:** Test 15b used production port 20128 while user-managed 9Router service was already listening, causing environment-dependent failure. **Resolution:** Refactored Tests 15a, 15b, and 15c to use isolated dynamically selected loopback ports (29114, 29115, 29117-29118) via new `TestPortOverrides` parameter. All 21 ecosystem tests now pass deterministically while preserving real 9Router on production port 20128. Also fixed Python mock fixtures in Tests 16-19 to use proper CMD batch file escaping for Windows compatibility.                                                                                                                                                               |
| 2            | `81398be070915e4c971bbd94aa5b89704b6fd548` | CHANGES_REQUIRED | **Finding 1:** CI format check failed. **Finding 2:** Tests 15a-15c hardcoded ports. **Finding 3:** Agent Scan metadata parsing not strict enough. **Finding 4:** Acceptance tests used source-text assertions. **Finding 5:** Review evidence incomplete. **Resolution:** (1) Fixed line endings. (2) Added `Get-AvailableLoopbackPort` using port 0 binding. (3) Strengthened pip metadata parsing with Tests 19a-19d. (4) Enhanced Tests 17, 18, 20, 21 with behavioral fixtures. (5) Updated review record.                                                                                                                                                                                                      |
| 3            | `cae2f1470aaa3578111ec45b3b71da609fa14801` | CHANGES_REQUIRED | **Finding 1:** TASK-AI-04.md formatting. **Finding 2:** Agent Scan missing exit code check. **Finding 3:** Test 20 installer not run from spaced path. **Finding 4:** Test 21 incomplete npm tools coverage. **Finding 5:** Test 15c port allocation race. **Finding 6:** Status and review record TBD. **Resolution:** (1) Fixed formatting. (2) Capture $LASTEXITCODE after pip show, reject on non-zero, added Test 19e. (3) Copy installer to spaced path and run from there. (4) Provide all 5 tools in npm JSON, assert all [OK], remove source-text checks. (5) Start first listener before allocating second port in Test 15c. (6) Status = READY_FOR_CODEX, Round 2 commit recorded. All 26 tests now pass. |
