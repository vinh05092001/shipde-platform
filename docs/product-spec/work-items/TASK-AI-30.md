...
Binding: this file must never declare `READY_FOR_CODEX` or later while the register records `BLOCKED_DEPENDENCY`; the Codex review record below is therefore `NOT_REVIEWED`. The block is the register's, not Git's: register row `162` (`work_item_id` `TASK-AI-29`) still records `BLOCKED_DEPENDENCY` with empty `pr` and empty `merge_commit`, so the queue carries no recorded delivery for the dependency and `AC-AI-30-03` asserts only the declared dependency name, never a merge. Conflict recorded, not silently resolved (`AGENTS.md` § Source of truth): `git log origin/main --grep=TASK-AI-29` returns two commits — `af16a1f` (PR #68, the specification, merged 2026-09-16) and `cd3cfd8` (PR #99, the implementation, merged 2026-09-17) — and the existing `acceptance/lib/dependency-merged.js` reports merge evidence from them (`dependencyProven('TASK-AI-29')` returns `ok: true` with 2 commits at the time of writing). The register governs the stage and the unreconciled row is the reconciler's to close, not this item's (`Do NOT touch … the register status column`); at the head of this branch after the F3 reconciliation merge of `origin/main` into it, `cd3cfd8` *is* an ancestor of this branch's head (`git merge-base --is-ancestor cd3cfd8 HEAD` exits 0), so the commit log and the register now disagree in the open: the register still governs the stage, row `162` still records no delivery, and closing that gap is still the governed reconciler's (`TASK-AI-19`) work, not this item's. The `READY_FOR_AUTHOR` and `IN_PROGRESS` transitions must be written by the governed reconciler (`TASK-AI-19`) before review routing. `AC-AI-30-01` compares the Control `Status` cell against register row `163` and fails on divergence.
...
| `python docs/product-spec/scripts/validate_docs.py` | `Documentation validation passed: 108 markdown files, 130 feature IDs, 178 delivery rows, 893 unique identifiers` |
| `node --test 'tools/ai-brain/test/*.test.js'` | `555` tests in `118` suites, `555` pass, `0` fail (the new file contributes `47`) |
| `node tools/ai-guard/cli.js secret-surface` | `SECRET_SURFACE_CLEAN`, `394` files scanned |
...
| 2 (repair) | `43b0342` | `NOT_REVIEWED` | Author-side repair of the #104 review at `c770258`: F1 fixed (`probeAccount` defaults the entry gate to `account-entry.entryFindings`, so the shipped CLI path refuses an entry-refused account before any spawn; proven by two tests that inject no gate). N1 fixed (default record moves to `~/.shipde/qualification-results`). F2, F3, F4, N2, N3 remain open. This repair was made on stacked PR #113's branch `fix/task-ai-30-entry-gate`, so `43b0342` is not itself an ancestor of this head; it reached this branch inside the squash commit `5b99604`, while `c770258` (the #104 review head) is an ancestor. |
| 3 (repair) | `3e7dffa` | `NOT_REVIEWED` | Author-side repair of the #113 review at `a175c51`: F2 fixed (`probeRuleFindings` in `acceptance/lib/qualification.js` requires entry admission, defaulting to `account-entry.entryFindings` matching `qualification.js:probeAccount`, refusing probe configurations with no entry admission or invalid account; doc specifies symbols covered by each row). N3 fixed (recorded conflict regarding TASK-AI-29 merge evidence on `origin/main` vs unreconciled register row 163 in ledger, preconditions, and acceptance notes). Absorbed #87 fail-closed `SOURCE_MISSING` operational guards into `ac-30-05`, `ac-30-07`, `ac-30-10`, `ac-30-11`, and `ac-30-13`; those five rows are byte-identical to `9afb45d`'s revisions of them at this head (`git diff 9afb45d HEAD` over the five paths is empty). The reviewed head `a175c51` and this repair commit live on stacked PR #113's branch `fix/task-ai-30-entry-gate` (base: this branch) and reached this branch inside `5b99604`, so neither is an ancestor of this head. |
| 4 (repair) | `5b99604` + merge of `origin/main` `29a48da` | `NOT_REVIEWED` | Author-side repair of the #104 review: F3 resolved by merging `origin/main` into this branch, which reconciles the duplicate delivery rather than rewriting history, and by taking `origin/main`'s revision of this Work Item and its fail-closed guards as the base of this file (the thirteen `ac-30-*` rows are byte-identical between the two parents, so nothing from #87 is regressed) — the ancestry claim in the ledger was corrected at the same time, since `cd3cfd8` is now an ancestor of this head. N2 fixed (a reused verdict reports `latencyMs: null` with `cached: true`; `validateResultShape` refuses a reused record carrying a measurement and a fresh record whose latency is not a finite non-negative number; the CLI says the verdict was reused instead of printing a latency nobody took). F2 evidence strengthened (a unit test asserts the acceptance rule and the shipped probe refuse the same entry-refused account). Counts and markers in this file refreshed at the merged head. |

## Residual limitations

- **The probe is delivered; no acceptance row performs a real provider call.** The bounded probe, the rule module (`acceptance/lib/qualification.js`), the declared result path and record shape (`tools/ai-brain/qualification.js`), the CLI surface and the unit tests are delivered in this branch, and rows `AC-AI-30-05` to `AC-AI-30-08` and `AC-AI-30-12` run green against them — but those rows exercise the rule and the shipped functions with injected doubles and a fake `exit 0` launch, so no row reads a real credential, calls a real provider or writes a real result. The provider call, the timeout and tree-kill, the cache window and the recorded outcome are implemented here and proven with doubles; what only an operator run against a real account can prove is that that account's own client answers. The rows above are the conditions that implementation must keep true, and they fail the moment the delivered modules stop satisfying the rule.
...
- **Branch naming history.** `#87` delivered the first revision of this file from `spec/task-ai-30`, which does not match the `feat/`- or `fix/`-prefixed work-item-id-and-slug rule in `AGENTS.md` § Unit of delivery — the same deviation as the earlier spec-only authoring branch `spec/task-ai-21`, merged as PR #74. That branch is merged to `origin/main` and is not rewritten; this branch, which carries the delivery, is `feat/task-ai-30-connection-test` (PR #104) and does conform.