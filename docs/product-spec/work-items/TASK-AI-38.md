...
| `AC-AI-38-16` | **Negative proof, must fail:** the baseline assertion `AC-AI-38-11` makes rejects warning creep, measured against the real audit rather than a synthetic one | `node tools/ai-brain/acceptance/ac-38-16-warning-creep.js` | `1` | `MANIFEST_BASELINE_DRIFT: expected exactly 1 warning, got 2` | `tools/ai-brain/acceptance/ac-38-16-warning-creep.js`; command stderr |
...
## AC-AI-38-16 was a tautology

The row generated its own findings in one `node -e` and asserted on them in a
second. Neither half read the repository.

Measured from an empty temporary directory containing no repository: it printed
`MANIFEST_BASELINE_DRIFT: expected exactly 1 warning, got 2` and exited `1` -
exactly the string and exit code the row demanded. It would have stayed green
with the whole project deleted, and it was still green on the day `AC-AI-38-08`
was repaired, because that repair looked at one row rather than at the pattern.

The replacement runs `node tools/ai-brain/cli.js manifest --json`, the same
command `AC-AI-38-11` uses, proves the real baseline is accepted as a control,
then adds one synthetic warning to a copy of the real findings and asserts the
same assertion refuses it. The check lives in a committed script rather than a
table cell, because a command that must survive markdown, then bash, then
PowerShell in CI can arrive mangled and die on `SyntaxError` with exit `1` - the
code this row expects, so a broken row would read as a passing one.

Measured after the change: exit `1` inside the repository with the expected
string, exit `2` outside it.

`AC-AI-38-05` and `AC-AI-38-06` were measured at the same time and are sound:
both call the real `auditManifest`, and both exit `1` rather than `0` when run
