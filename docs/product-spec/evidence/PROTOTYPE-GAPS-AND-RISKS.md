...
| **Prisma Schema Drift** | `prisma/schema.prisma` is well-formed with 12 domain models, but has no applied migrations or active connection to a running database container. | `HIGH` | `TASK-FOUND-03` |
...
4. **Database Schema (`prisma/schema.prisma`):**
   - 12 PostgreSQL models with multi-tenant foreign keys, indexes, unique constraints, and optimistic locking fields (`version`).
