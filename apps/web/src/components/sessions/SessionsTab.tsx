...
export function SessionsTab({ onToast }: SessionsTabProps) {
  ...
  const fetchSessions = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) { setSessions(data.data || []); setFetchState('success'); }
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    if (!lookupId.trim()) { setLookupError('Please enter a session ID'); return; }
    if (lookupId.trim().length < 3) { setLookupError('Session ID must be at least 3 characters'); return; }
    setLookupLoading(true);
    try {
      const res = await fetch('/api/sessions?status=active');
      if (res.ok) {
        const data = await res.json();
        const found = (data.data || []).find((s: SessionItem) => s.id.toLowerCase() === lookupId.trim().toLowerCase());
        if (found) setLookupResult(found);
        else setLookupError('Session not found: ' + lookupId.trim());
      } else { setLookupError('Cannot lookup session'); }
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 space-y-4">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl animate-pulse">
              ...
              <div className="flex-1 space-y-2"><div className="h-3 bg-slate-200 rounded w-1/3" /><div className="h-3 bg-slate-200 rounded w-1/2" /></div>
            </div>
          ))}
        </div>
      </div>
    ...
    return (
      <div className="space-y-4">
        <div className="bg-white rounded-2xl border border-slate-200 p-10 text-center">
          ...
          <p className="text-sm text-slate-500 font-medium mb-6">Session list is empty. New login devices will appear here.</p>
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">{activeSessions.length} active / {sessions.length} total</p>
            </div>
          </div>
          <button type="button" onClick={() => setRevokeAllOpen(true)} disabled={!hasActiveSessions || safetyCheck()}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5">
        ...
        <form onSubmit={handleLookup} className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 flex gap-2">
            <input type="text" value={lookupId} onChange={(e) => { setLookupId(e.target.value); setLookupError(null); setLookupResult(null); }}
              placeholder="Enter session ID (e.g. dev_abc123)..." className="flex-1 modern-input font-mono text-sm" />
        ...
        {lookupResult && (
          <div className="mt-3 flex items-start gap-2 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3">
            ...
            <div className="font-medium">Found: <span className="font-mono">{lookupResult.id}</span> - {lookupResult.is_revoked ? <span className="text-red-600 font-bold">Revoked</span> : <span className="text-emerald-600 font-bold">Active</span>}</div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        <div className="p-5 border-b border-slate-100">
          <h3 className="text-sm font-black text-slate-800">Session List ({activeSessions.length} active / {sessions.length} total)</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead><tr className="bg-slate-50">
              <th className="px-5 py-3 text-xs font-black text-slate-600 uppercase tracking-wider">Device</th>
              <th className="px-5 py-3 text-xs font-black text-slate-600 uppercase tracking-wider">User</th>
              <th className="px-5 py-3 text-xs font-black text-slate-600 uppercase tracking-wider">Status</th>
              <th className="px-5 py-3 text-xs font-black text-slate-600 uppercase tracking-wider">Last Active</th>
              <th className="px-5 py-3 text-xs font-black text-slate-600 uppercase tracking-wider text-center">Action</th>
            </tr></thead>
            <tbody className="divide-y divide-slate-100">
              {sessions.map((session) => (
                <tr key={session.id} className="hover:bg-slate-50 transition">
                  <td className="px-5 py-4"><div className="flex items-center gap-2"><div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center"><ShieldCheck className={`w-4 h-4 ${session.is_revoked ? 'text-slate-400' : 'text-emerald-500'}`} /></div><span className="text-sm font-semibold text-slate-800">{session.device}</span></div></td>
                  <td className="px-5 py-4"><span className="text-sm font-medium text-slate-700">{session.user_name}</span></td>
                  <td className="px-5 py-4">{session.is_revoked ? <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-red-50 text-red-700 rounded-full text-xs font-bold"><X className="w-3 h-3" /> Revoked</span> : <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-full text-xs font-bold"><CheckCircle2 className="w-3 h-3" /> Active</span>}</td>
                  <td className="px-5 py-4"><span className="text-sm text-slate-500 font-medium">{session.last_active}</span></td>
                  <td className="px-5 py-4 text-center">{!session.is_revoked && <button type="button" className="text-xs font-bold text-red-600 hover:text-red-800 hover:bg-red-50 px-3 py-1.5 rounded-lg transition cursor-pointer" title="Revoke this session">Revoke</button>}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div><h3 className="text-base font-black text-slate-900">Confirm Revoke</h3><p className="text-xs text-slate-500">Revoke all other sessions</p></div>
            </div>
            {safetyCheck() ? (
              <div className="flex items-start gap-2 text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                <AlertCircle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
                <span className="font-medium">This is the only active session. Cannot revoke all.</span>
              </div>
            ) : (
              <p className="text-sm text-slate-600">This will revoke <strong>{activeSessions.length - 1}</strong> other sessions (excluding current). Users on those devices will be logged out.</p>
            )}
            <div className="flex justify-end gap-3">
              <button type="button" onClick={() => setRevokeAllOpen(false)} className="px-4 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-xl transition cursor-pointer">Cancel</button>
              <button type="button" onClick={handleRevokeAll} disabled={safetyCheck() || revokeAllLoading}
                ...
                {revokeAllLoading ? <span className="inline-flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> Revoking...</span> : 'Revoke All'}
              </button>
            </div>
          </div>
        </div>
