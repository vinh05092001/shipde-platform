...
export async function POST(req: NextRequest) {
  try {
    ...
    const { action, carrier_code, account_name, token, client_id, capability_tier, account_id } =
      body;

    // Toggle / Update Capability Tier (E2E-10, BR-19)
    if (action === 'TOGGLE_TIER') {
      const acc = db.carrierAccounts.find(
        (a) => a.id === account_id || a.carrier_code === carrier_code
      );
      if (!acc) {
        return NextResponse.json(
          { error: { code: 'account_not_found', message: 'Không tìm thấy tài khoản hãng' } },
          { status: 404 }
        );
      }
    ...
    if (action === 'CONNECT_CARRIER') {
      if (!carrier_code || !token) {
        return NextResponse.json(
          {
            error: { code: 'validation_error', message: 'Vui lòng cung cấp mã hãng và API Token' },
          },
          { status: 400 }
        );
      }
