...
import { MASTER_SHIPMENTS, MASTER_EXCEPTIONS, MASTER_DISCREPANCIES, MASTER_CLAIMS, getUnifiedMetrics } from '../services/unifiedDataStore';
...
async function runRegressionSuite() {
  ...
  assert(metrics.totalShipments === 50, `Tổng số bưu kiện trong kho dữ liệu chuẩn là 50 (Thực tế: ${metrics.totalShipments})`);
  assert(MASTER_SHIPMENTS.length === 50, `Danh sách bưu kiện đồng bộ 100% với metrics (50 bản ghi)`);
  assert(metrics.openExceptionsCount === MASTER_EXCEPTIONS.filter(e => e.status === 'OPEN' || e.status === 'ASSIGNED').length, `Badge Hộp việc và danh sách sự cố mở khớp số lượng (${metrics.openExceptionsCount})`);
  assert(metrics.openDiscrepanciesCount === MASTER_DISCREPANCIES.filter(d => d.status === 'OPEN').length, `Badge Khoản lệch và bảng đối soát khớp số lượng (${metrics.openDiscrepanciesCount})`);
  ...
  try {
    ...
    if (err.code === 'self_approval_forbidden' || err.message?.includes('tách quyền') || err.message?.includes('không thể tự duyệt')) {
      selfApprovalBlocked = true;
    }
  }
  assert(selfApprovalBlocked, 'Quy tắc BR-12: Chặn tuyệt đối người tạo (CSKH) tự duyệt khoản chênh lệch tài chính');
  ...
  assert(resolved.status === DiscrepancyResolution.CONFIRMED && resolved.version === 2, 'Kế toán viên độc lập duyệt chênh lệch thành công và tăng version lên 2');

  // Test 3: Total Discrepancy Sum Consistency
  const sumAmount = MASTER_DISCREPANCIES.reduce((sum, d) => sum + d.discrepancy_amount, 0);
  assert(sumAmount === 1978000, `Tổng số tiền lệch D1..D7 tính toán chính xác từ từng dòng: 1.978.000 đ (Thực tế: ${sumAmount.toLocaleString('vi-VN')} đ)`);
