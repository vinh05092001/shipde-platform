...
export const CreateOrderModal: React.FC<Props> = ({ isOpen, onClose, onOrderCreated }) => {
  ...
  const [pickupWarehouse, setPickupWarehouse] = useState('Kho Tân Bình (123 Cộng Hòa, P.12, Tân Bình, HCM)');
  ...
  const [recipientAddress, setRecipientAddress] = useState('123 Nguyễn Thị Minh Khai, P.6');
  ...
  // Selected Carrier
  const [selectedCarrier, setSelectedCarrier] = useState<'GHN' | 'GHTK' | 'VTP' | 'JT'>('GHN');
  ...
  const carriersComparison = [
    {
      ...
      codFee: 0, // Free COD policy
      deliveryTime: isIntraProvince ? 'Ngày mai (18/08) · 24h' : '1 - 2 ngày',
      tier: 'L2',
      tierLabel: 'L2 Execute (Bắn API trực tiếp)',
    ...
    {
      id: 'VTP',
      name: 'Viettel Post (VTP Express)',
      baseFee: isIntraProvince ? 21500 : 31000,
      addWeightFee: chargeableWeightGram > 1000 ? Math.ceil((chargeableWeightGram - 1000) / 500) * 4500 : 0,
      insuranceFee: insuranceValue > 1000000 ? Math.round(insuranceValue * 0.005) : 0,
      codFee: 0,
      deliveryTime: isIntraProvince ? '1 - 2 ngày' : '2 - 3 ngày',
      tier: 'L2',
      tierLabel: 'L2 API Trực Tiếp',
      badge: '💰 Cước Rẻ Nhất',
    ...
    },
    {
      ...
      deliveryTime: isIntraProvince ? '1 - 2 ngày' : '2 - 3 ngày',
      tier: 'L1',
      tierLabel: 'L1 Assist (Hồ Sơ Hỗ Trợ)',
      badge: 'Phổ Biến',
    ...
    },
    {
      ...
      baseFee: isIntraProvince ? 23000 : 33000,
      addWeightFee: chargeableWeightGram > 1000 ? Math.ceil((chargeableWeightGram - 1000) / 500) * 5000 : 0,
      insuranceFee: insuranceValue > 1000000 ? Math.round(insuranceValue * 0.005) : 0,
      codFee: 0,
      deliveryTime: isIntraProvince ? '1 - 2 ngày' : '2 - 3 ngày',
    ...
    try {
      const res = await fetch('/api/orders', {
        ...
        body: JSON.stringify({
          ...
          recipient_province: recipientProvince === 'HCM' ? 'TP. Hồ Chí Minh' : recipientProvince === 'HN' ? 'Hà Nội' : 'Đà Nẵng',
          ...
          estimated_fee: activeCarrierInfo.totalFee,
        }),
      });
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl border border-slate-200 max-w-5xl w-full max-h-[92vh] flex flex-col overflow-hidden shadow-2xl text-xs animate-in zoom-in-95 duration-150">
        {/* Modal Header */}
        <div className="p-4 sm:p-5 border-b border-slate-100 bg-slate-50/70 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-sm text-slate-900">Tạo Đơn Hàng & So Sánh Cước Phí Đa Hãng</h3>
                <span className="badge-ok text-[10px]">Auto Rate Comparison</span>
              </div>
              <p className="text-[11px] text-slate-500 mt-0.5">
                Nhập thông tin giao hàng để hệ thống tự động tính cước và gợi ý hãng vận chuyển tối ưu nhất
              </p>
            </div>
          </div>
          <button
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="space-y-3 p-4 bg-slate-50/70 rounded-xl border border-slate-200/80">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-[11px] font-mono text-slate-400">Mã đơn: {orderCode}</span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    >
                      ...
                      <option value="OTHER">Tỉnh thành khác (Liên Tỉnh)</option>
                    </select>
                  </div>
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Quận / Huyện:</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="font-semibold text-slate-700 block mb-1">Địa Chỉ Chi Tiết (Số nhà, Tên đường, Phường):</label>
                  <input
                    type="text"
                    value={recipientAddress}
                    onChange={(e) => setRecipientAddress(e.target.value)}
                    placeholder="123 Nguyễn Thị Minh Khai, Phường 6"
                    className="modern-input w-full text-xs"
                    required
                  />
                </div>
              ...
              <div className="space-y-3 p-4 bg-slate-50/70 rounded-xl border border-slate-200/80">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Cân Nặng (g):</label>
                    <input
                      type="number"
                      value={weightGram}
                  ...
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Dài (cm):</label>
                    <input
                      type="number"
                      value={lengthCm}
                  ...
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Rộng (cm):</label>
                    <input
                      type="number"
                      value={widthCm}
                  ...
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Cao (cm):</label>
                    <input
                      type="number"
                      value={heightCm}
                ...
                <div className="flex justify-between items-center p-2.5 bg-blue-50/60 rounded-lg border border-blue-100 text-[11px] text-blue-900">
                  <span>Cân thực: <strong>{weightGram}g</strong> · Thể tích quy đổi (D×R×C/5000): <strong>{volumetricWeightGram}g</strong></span>
                  <span className="font-bold text-blue-700">Trọng lượng tính cước: {chargeableWeightGram}g</span>
                </div>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Tiền Thu Hộ COD (VND):</label>
                    <input
                      type="number"
                      value={codAmount}
                  ...
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Khai Giá Bảo Hiểm:</label>
                    <input
                      type="number"
              ...
              <div className="grid grid-cols-2 gap-3 p-3 bg-slate-50/70 rounded-xl border border-slate-200/80">
                <div>
                  <label className="font-semibold text-slate-700 block mb-1">Lưu Ý Xem Hàng:</label>
                  <select
                ...
                <div>
                  <label className="font-semibold text-slate-700 block mb-1">Người Trả Cước Phí:</label>
                  <select
                    value={payer}
            ...
            <div className="lg:col-span-5 space-y-4 flex flex-col justify-between">
              <div className="space-y-3">
                <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                  <div className="flex items-center gap-1.5 font-bold text-slate-900 text-xs">
                    ...
                    <span>So Sánh Cước Phí Đa Hãng Thời Gian Thực:</span>
                  </div>
                  <span className="text-[10px] text-slate-400 font-mono">4 Hãng Kết Nối</span>
                </div>
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex items-center gap-2.5">
                            <div className={`w-8 h-8 rounded-lg ${carrier.logoBg} flex items-center justify-center font-bold text-xs shrink-0`}>
                              {carrier.id}
                            </div>
                            <div>
                              ...
                              <div className="flex items-center gap-2 text-[10px] text-slate-500 mt-0.5">
                                ...
                                <span>·</span>
                                <AutomationBadge tier={carrier.tier as any} />
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="text-right">
                            ...
                            <span className={`badge-muted text-[9px] font-bold ${carrier.badgeColor}`}>
                              {carrier.badge}
                            </span>
                          </div>
                        </div>
                        ...
                        {isSelected && (
                          <div className="pt-2 border-t border-blue-100 flex justify-between items-center text-[10px] text-blue-900 font-medium">
                            <span>Cước gốc: <Money amount={carrier.baseFee} state="confirmed" className="inline" /></span>
                            <span>Phụ phí cân: <Money amount={carrier.addWeightFee} state="confirmed" className="inline" /></span>
                            <span>Bảo hiểm: {carrier.insuranceFee ? `${carrier.insuranceFee.toLocaleString()}đ` : 'Miễn phí'}</span>
                          </div>
                        )}
                      </div>
                    );
              ...
              <div className="p-4 bg-slate-900 text-white rounded-2xl space-y-3 shadow-lg">
                <div className="flex justify-between items-center border-b border-slate-800 pb-2 text-xs">
                  <span className="text-slate-400">Hãng được chọn:</span>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Tổng Cước Vận Chuyển:</span>
                    <div className="font-mono text-2xl font-black text-emerald-400">
                      <Money amount={activeCarrierInfo.totalFee} state="confirmed" className="text-emerald-400" />
                    </div>
                  </div>

                  <div className="text-right">
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider block">Tiền Thu COD:</span>
                    <div className="font-mono text-base font-bold text-white">
                      <Money amount={codAmount} state="confirmed" className="text-white" />
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={loading}
                  className="btn-primary w-full h-11 justify-center text-xs font-bold bg-blue-600 hover:bg-blue-500 shadow-md shadow-blue-500/25"
                >
                  {loading ? (
                    <span className="flex items-center gap-2">
                      <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                      <span>Đang gọi API tạo vận đơn...</span>
                    </span>
                  ) : (
